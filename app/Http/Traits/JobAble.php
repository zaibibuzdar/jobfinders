...
trait JobAble
{
    ...
    private function filterJobs($request)
    {
        ...
        // Set default radius to 10 if not provided
        $radius = $request->has('radius') && $request->radius != null ? $request->radius : 10;

        if ($request->has('lat') && $request->has('long') && $request->lat != null && $request->long != null) {
            $lat = $request->lat;
            $long = $request->long;

            // Apply the radius filter with the provided or default radius (10 km)
            $query->whereRaw(
